/*
 * Fused SSM Recurrence Kernels (Issue #31, #53)
 *
 * Fuses the entire sequence loop into a single CUDA kernel.
 * Grid: (batch,)  — one block per batch element, multiple threads per block
 * Each thread handles one or more independent work items (dimensions, blocks,
 * or pairs) via stride loop: for (int i = threadIdx.x; i < N; i += blockDim.x)
 * Loop: sequential over T timesteps within the kernel (recurrence is serial)
 *
 * Four variants:
 ...
 *   4. pascal_forward: per-dim decay + quaternion rotation (grade 1 + grade 2)
 *
...
__global__ void quatblock_forward_kernel(
    ...
    // Each thread handles one or more 3D blocks via stride loop
...
__global__ void givens_forward_kernel(
    ...
    // Each thread handles one or more SO(2) pairs via stride loop
...
__global__ void diagonal_forward_kernel(
    ...
    // Each thread handles one or more dimensions via stride loop
...
torch::Tensor quatblock_forward(
    ...
    const int threads = std::min(n_blocks, 256);
    quatblock_forward_kernel<<<B, threads>>>(
...
torch::Tensor givens_forward(
    ...
    const int threads = std::min(n_pairs, 256);
    givens_forward_kernel<<<B, threads>>>(
...
torch::Tensor diagonal_forward(
    ...
    const int threads = std::min(D, 256);
    diagonal_forward_kernel<<<B, threads>>>(
        decays.data_ptr<float>(),
        injections.data_ptr<float>(),
        states.data_ptr<float>(),
        T, D
...
torch::Tensor pascal_forward(
    ...
    const int threads = std::min(n_blocks, 256);
    pascal_forward_kernel<<<B, threads>>>(
        bivectors.data_ptr<float>(),
        decays.data_ptr<float>(),
        injections.data_ptr<float>(),
        states.data_ptr<float>(),
